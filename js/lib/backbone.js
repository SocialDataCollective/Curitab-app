...
(function (factory) {
	...
	_.extend(Collection.prototype, Events, {
		...
		set: function (models, options) {
			...
			for (var i = 0; i < models.length; i++) {
				...
				if (order && (model.isNew() || !modelMap[id])) {
					...
					orderChanged = orderChanged || !this.models[i] || model.cid !== this.models[
						i].cid;
				}
			...
			if (!options.silent) {
				...
				if (toAdd.length || toRemove.length) this.trigger('update', this,
					options);
			}
		...
		create: function (model, options) {
			...
			options.success = function (model, resp, callbackOpts) {
				...
				if (success) success.call(callbackOpts.context, model, resp,
					callbackOpts);
			};
	...
	_.extend(History.prototype, Events, {
		...
		start: function (options) {
			...
			var addEventListener = window.addEventListener || function (eventName,
				listener) {
